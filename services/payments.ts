...
export async function initializeChapaCheckout(payload: CheckoutPayload) {
  ...
  const txRef = await chapa.genTxRef({ prefix: "MEDA", size: 20 });
  ...
  const payment = await prisma.payment.create({
    data: {
      ...
      // Keeping legacy column names for now to avoid requiring a DB migration.
...
export async function confirmChapaPayment(payload: ConfirmPayload) {
  ...
  const chapa = getChapaClient();
  const verification = await chapa.verify({ tx_ref: payload.txRef });
  ...
  if (
    payment.event.capacity != null &&
    currentCount + quantity > payment.event.capacity
  ) {
    throw new Error("Not enough seats available");
  }
