import { NextResponse } from "next/server";
import { requireSessionUser } from "@/lib/auth/guards";
import { formatUnknownError } from "@/lib/apiResponse";
import { logger } from "@/lib/logger";
import {
  partyIdParamSchema,
  partyPatchSchema,
} from "@/lib/validations/bookingInventory";
import {
  parseJsonBody,
  parseParams,
  validationErrorResponse,
} from "@/lib/validations/http";
import { getPartyForUser, updateParty } from "@/services/parties";

export async function GET(
  _request: Request,
  context: { params: Promise<{ id: string }> },
) {
  const sessionCheck = await requireSessionUser();
  if (sessionCheck.response) return sessionCheck.response;

  const parsed = parseParams(partyIdParamSchema, await context.params);
  if (!parsed.success) {
    return validationErrorResponse(parsed.error, "Invalid party id");
  }

  try {
    const party = await getPartyForUser({
      partyId: parsed.data.id,
      userId: sessionCheck.user!.id,
      userEmail: sessionCheck.user!.email ?? null,
    });
    return NextResponse.json({ party }, { status: 200 });
  } catch (error) {
    logger.error("Failed to load party", error);
    return NextResponse.json(
      { error: formatUnknownError(error) },
      { status: 404 },
    );
  }
}

export async function PATCH(
  request: Request,
  context: { params: Promise<{ id: string }> },
) {
  const sessionCheck = await requireSessionUser();
  if (sessionCheck.response) return sessionCheck.response;

  const parsedParams = parseParams(partyIdParamSchema, await context.params);
  if (!parsedParams.success) {
    return validationErrorResponse(parsedParams.error, "Invalid party id");
  }

  const parsedBody = await parseJsonBody(partyPatchSchema, request);
  if (!parsedBody.success) {
    return validationErrorResponse(parsedBody.error, "Invalid party update");
  }

  try {
    const party = await updateParty({
      partyId: parsedParams.data.id,
      ownerId: sessionCheck.user!.id,
      name: parsedBody.data.name,
      status: parsedBody.data.status,
    });
    return NextResponse.json({ party }, { status: 200 });
  } catch (error) {
    logger.error("Failed to update party", error);
    return NextResponse.json(
      { error: formatUnknownError(error) },
      { status: 400 },
    );
  }
}
