...
import { getOwnerDashboardUtilization } from "@/services/ownerAnalytics";
...
export async function GET(request: Request) {
  ...
  if (!parsed.success) {
    return validationErrorResponse(parsed.error, "Invalid owner utilization query");
  }

  try {
    const utilization = await getOwnerDashboardUtilization({
      ownerId: sessionCheck.user!.id,
      pitchId: parsed.data.pitchId,
      from: parsed.data.from ? new Date(parsed.data.from) : undefined,
      to: parsed.data.to ? new Date(parsed.data.to) : undefined,
    });
    return NextResponse.json({ utilization }, { status: 200 });
  } catch (error) {
    logger.error("Failed to load owner utilization", error);
