...
import { listOwnerDashboardBookings } from "@/services/ownerAnalytics";
...
export async function GET(request: Request) {
  ...
  if (!parsed.success) {
    return validationErrorResponse(parsed.error, "Invalid owner booking query");
  }

  try {
    const bookings = await listOwnerDashboardBookings({
    ...
    logger.error("Failed to load owner bookings", error);
