...
import { getOwnerDashboardOverview } from "@/services/ownerAnalytics";
...
export async function GET(request: Request) {
  ...
  if (!parsed.success) {
    return validationErrorResponse(parsed.error, "Invalid owner overview query");
  }
  ...
  try {
    ...
    logger.error("Failed to load owner overview", error);
