    ...
    <PageShell containerClassName="mx-auto flex max-w-5xl flex-col gap-8 px-4 py-10 sm:py-12">
      <section className="glass-card space-y-4 rounded-3xl border border-[var(--color-border)] bg-[rgba(10,24,38,0.86)] p-6 sm:p-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-bold text-white sm:text-4xl">Answers for players and hosts</h1>
            <p className="max-w-3xl text-sm leading-relaxed text-[var(--color-text-secondary)] sm:text-base">
              Everything you need to move from "I found a match" to "See you on the pitch" with confidence.
            </p>
          </div>
          <Link href="/events" className={cn(buttonVariants("secondary", "lg"), "rounded-full px-6")}>
            Browse events
          </Link>
        </div>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <article className="glass-card space-y-3 rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
          ...
          <div className="space-y-2 text-sm text-[var(--color-text-secondary)]">
            ...
            <p>Wait a moment and check if a ticket issued. If not, retry with a stable connection; no charge is made without a ticket.</p>
            <p className="font-semibold text-white">Didn't receive email</p>
            <p>Look in spam, then open your ticket from Profile -> My Tickets. Email delivery can lag; the in-app ticket is instant.</p>
            <p className="font-semibold text-white">Can't scan QR at the gate</p>
            <p>Use brightness max, and ensure the ticket hasn't been shared. Hosts can search your name as a backup.</p>
          </div>
        </article>
      </section>
      ...
      <section className="glass-card space-y-4 rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
        ...
        <div className="space-y-3">
          {faqs.map((item) => (
            <article key={item.q} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-2)] p-4">
      ...
      <section className="grid gap-4 sm:grid-cols-2">
          ...
          <ul className="space-y-2 text-sm text-[var(--color-text-secondary)]">
            {safety.map((item) => (
              <li key={item} className="flex gap-2">
                <span className="mt-1 inline-flex h-2 w-2 rounded-full bg-[var(--color-brand-alt)]" aria-hidden />
                <span>{item}</span>
              </li>
            ))}
          </ul>
        ...
        <article className="glass-card space-y-3 rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
          ...
          <p className="text-sm text-[var(--color-text-secondary)]">
            ...
            with your account email, event ID, payment reference (if any), and a quick summary. We respond with concrete next steps.
          </p>
          <div className="flex flex-wrap gap-3 text-sm">
            <Link href="/terms" className="rounded-full border border-[var(--color-border)] px-3 py-1 text-[var(--color-text-primary)] hover:border-[var(--color-brand)] hover:text-[var(--color-brand)]">
              Terms &amp; Conditions
            </Link>
            <Link href="/privacy" className="rounded-full border border-[var(--color-border)] px-3 py-1 text-[var(--color-text-primary)] hover:border-[var(--color-brand)] hover:text-[var(--color-brand)]">
              Privacy Policy
            </Link>
          </div>
        </article>
